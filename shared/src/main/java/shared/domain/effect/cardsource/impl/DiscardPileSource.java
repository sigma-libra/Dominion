...
/**
 * CardSource: the cards in the player's discard pile
 */
public class DiscardPileSource implements CardSource, Serializable {

    private static final Logger LOG = LoggerFactory.getLogger(MethodHandles.lookup().lookupClass());

    public List<Integer> getChoices(GameState gameState, Player player) {
        LOG.info("getChoices");
        return player.getDiscard().stream().map(c -> c.getID()).collect(Collectors.toList());
    }
    ...
    public CardPile getDestination(GameState gameState, Player player) {
